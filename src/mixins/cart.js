...
export default class cartMixin extends wepy.mixin {
   ...
   */
    getCartList(callback) {
        ...
        if (this.isFunction(callback)) {
            ...
            // 更新返回的列表
            this.isArray(result) && this
                .$parent
                .$updateGlobalData('cart', result);
        }
        return list;
    }
   ...
   */
    updateCart({
        ...
        // 整理数据
        const itemArr = this.isArray(arr)
            ? arr
            : [arr];
        ...
        itemArr.map((good) => {
            ...
            this.isFunction(getList) && getList.bind(this)((list) => {
                ...
                // 存在时，add增加数量，remove直接删除 不存在时，add直接增加
                if (isRemove) {
                    // remove只在存在时操作删除
                    this.isDefined(existIndex) && list.splice(existIndex, 1);
                } else {
                    // add: 存在加数量，不存在push
                    this.isDefined(existIndex)
                        ? list[existIndex].amount++
                        : list.push(tempData);
                }
                return list;
            });
